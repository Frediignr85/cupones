...
use App\Models\ModeloOfertasPendientes;
use App\Models\ModeloDashboard;
use Config\App;
use \Hermawan\DataTables\DataTable;
header('Access-Control-Allow-Origin: *');
class Ofertas_Pendientes extends BaseController
{
    function __construct()
    {
        $this->ElModelo = new ModeloOfertasPendientes();
    ...
    public function index()
    {
        ...
        if($session->get('id_usuario') == ""){
            ...
            $links = $modelo->verificar_permiso($session->get('id_usuario'),"ofertas_pendientes/");

            $query = $modelo->datos_empresa(1);
            $datos['result'] = $query->getResultArray();
            if ($links!='NOT' || $admin=='1' ){
                $datos1['title'] = 'Admin Ofertas Pendientes';
                $datos1['links'] = null;
                ...
                $query = $modelo->ofertas_pendientes($session->get('admin'),$session->get('id_empresa'));
                ...
                $links2 = $modelo->verificar_permiso($session->get('id_usuario'),"ofertas/editar_oferta");
                ...
                $datos2['permiso_rechazar'] = 0;
                $links2 = $modelo->verificar_permiso($session->get('id_usuario'),"rechazar_oferta");
                if ($links2!='NOT' || $admin=='1' ){
                    $datos2['permiso_rechazar'] = 1;
                }
                $datos2['permiso_descartar'] = 0;
                $links2 = $modelo->verificar_permiso($session->get('id_usuario'),"descartar_oferta");
                if ($links2!='NOT' || $admin=='1' ){
                    $datos2['permiso_descartar'] = 1;
                }
                ...
                echo view('admin_ofertas_pendientes',$datos2);
