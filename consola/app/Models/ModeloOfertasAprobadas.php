...
class ModeloOfertasAprobadas extends Model
{
    ...
    function ofertas_aprobadas(){
        $data = $this->db->query("SELECT tbloferta.id_oferta, tblempresa_ofertante.nombre, tbloferta.titulo_oferta, tbloferta.cantidad_limite_cupones, tbloferta.precio_regular, tbloferta.precio_oferta, tbloferta.fecha_inicio, tbloferta.fecha_fin, tbloferta.fecha_limite, (tblempresa_ofertante.porcentaje * tbloferta.precio_oferta /100) as 'comision', tblempresa_ofertante.id_empresa FROM tbloferta INNER JOIN tblempresa_ofertante on tbloferta.id_empresa = tblempresa_ofertante.id_empresa WHERE tbloferta.id_estado = 2");
        return $data;
    }
